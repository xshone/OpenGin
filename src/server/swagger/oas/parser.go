...
func newParser(modulePath, mainFilePath, handlerPath string, debug, withoutPkgName bool) (*parser, error) {
	p := &parser{
		KnownPkgs:               []pkg{},
		KnownNamePkg:            map[string]*pkg{},
		KnownPathPkg:            map[string]*pkg{},
...
func (p *parser) parseOperation(pkgPath, pkgName string, astComments []*ast.Comment) error {
	...
	for _, astComment := range astComments {
		...
		case "@description":
			...
			// TODO: 修复换行符以支持Markdown形式的Description
			value := strings.Replace(comment[len(attribute):], "\\n", "\n", -1)
			operation.Description = strings.Join([]string{operation.Description, strings.TrimSpace(value)}, " ")
		case "@param":
		...
		case "@security":
			// TODO: 增加对Security注释的处理
			err = p.parseSecurityComment(operation, strings.TrimSpace(comment[len(attribute):]))
		}
...
// TODO: parseSecurityComment
func (p *parser) parseSecurityComment(operation *OperationObject, comment string) error {
	commentSlice := strings.SplitN(comment, " ", -1)

	if len(commentSlice) >= 1 {
		schemaName := commentSlice[0]
...
func (p *parser) parseParamComment(pkgPath, pkgName string, operation *OperationObject, comment string) error {
	...
	// TODO: 修改form为application/x-www-form-urlencoded，multiform为multipart/form-data
	// `form`
	if in == "form" {
		...
		if isGoTypeOASType(goType) {
			operation.RequestBody.Content[ContentTypeMultiForm].Schema.Properties.Set(name, &SchemaObject{
			...
			if required {
				operation.RequestBody.Content[ContentTypeMultiForm].Schema.Required = append(operation.RequestBody.Content[ContentTypeMultiForm].Schema.Required, name)
			}
		} else {
...
func (p *parser) registerType(pkgPath, pkgName, typeName string) (string, error) {
	...
	} else if _, ok := p.KnownIDSchema[genSchemeaObjectID(pkgName, typeName, p.WithoutPkgName)]; ok {
		return genSchemeaObjectID(pkgName, typeName, p.WithoutPkgName), nil
	} else {
...
func (p *parser) parseSchemaObject(pkgPath, pkgName, typeName string) (*SchemaObject, error) {
	...
	if strings.HasPrefix(typeName, "[]") {
		...
		schema, ok := p.KnownIDSchema[genSchemeaObjectID(pkgName, itemTypeName, p.WithoutPkgName)]
		if ok {
	...
	if len(typeNameParts) == 1 {
		...
		schemaObject.ID = genSchemeaObjectID(pkgName, typeName, p.WithoutPkgName)
		...
		if !exist {
			...
			schemaObject.ID = genSchemeaObjectID(guessPkgName, guessTypeName, p.WithoutPkgName)
			p.KnownIDSchema[schemaObject.ID] = &schemaObject
		}
